...
for df in (df_com, df_sin, df_hosp):
    ...
    predict(X, y, lambda: KNeighborsClassifier(n_neighbors=10), name="knn/cdmx/"+df.name+".txt")

from gen_data import gen_data_mx, predict  
df_com, df_hosp = gen_data_mx()
for df in (df_com, df_hosp):
    X, y = df.iloc[:, 1:], df.iloc[:, 0]
    predict(X, y, lambda: KNeighborsClassifier(n_neighbors=10), name="knn/mx/"+df.name+".txt")